...
l'étape de pruning unifiée (module [`prune-codes/`](../prune-codes/)) :

```
prune ──→ index-notices ──→ classify-rag-notices
```

> Le pruning (nomenclature, mapping, annotations, suggester) est désormais centralisé
> dans le module **`prune-codes/`** ; les anciens scripts `0_prunning_coicop.py` et
...
de l'étape `prune-codes` (`prune-codes/annotations_test_pruned.parquet`, `prune-codes/nomenclature_pruned.parquet`,
`prune-codes/mapping_lvl4.parquet`), la collection Qdrant `coicop_lineage` et le prompt Langfuse
