...
|---|---|---|
| `nomenclature_pruned.parquet` | nomenclature prunée (niveaux 1 à 4) | `index-notices` (rag-notices) |
| `mapping_lvl4.parquet` | table `code → code_parent_equivalent` | scoring `classify-rag-notices` |
| `annotations_train_pruned.parquet` | KB d'annotations prunées | `index-annotations` |
| `annotations_test_pruned.parquet` | jeu à coder pruné | `classify-rag-notices` et `classify-rag-annotations` |
| `suggester_pruned.parquet` | suggester pruné | `index-annotations` |
...
Entrées : la nomenclature COICOP brute + les sorties de `classify-regex`
