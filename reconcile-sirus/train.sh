...
# (L'entraînement produit un modèle pour un run futur ; cf. reconcile-sirus/README.md.)
#
...
uv run main.py build-table \
  ...
  --rag-annotations-file "$ROOT/classify-rag-annotations/predictions.parquet" \
  --ttc-file "$ROOT/classify-ttc/predictions.parquet" \
  --mapping-file "$ROOT/prune-codes/mapping_lvl4.parquet" \
  --tocodify-file "$ROOT/classify-regex/raw_test_without_regex.parquet" \
