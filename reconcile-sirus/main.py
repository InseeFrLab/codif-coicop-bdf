...
from reconcile_llm import _read_parquet, _write_parquet, load_all_observations  # noqa: E402
...
def _merged_table(args: argparse.Namespace) -> pd.DataFrame:
    """Fusion des 4 classifieurs + troncature niveau 4, via `reconcile-llm`.
...
def _share_without_candidate(artifacts_dir: Path) -> float | None:
    ...
    Sert de référence au contrôle de dérive de `reconcile-sirus` : mesurée sur un
...
def cmd_finalize(args: argparse.Namespace) -> int:
    ...
    payload["training"] = {
        ...
        # Distributions d'entraînement : `reconcile-sirus` les compare à celles du
    ...
    logger.info("    reconcile-sirus-model-uri: %s", uri)
    logger.info("=" * 72)
    logger.info(
...
# --------------------------------------------------------------------------- #
# predict : étape reconcile-sirus, Python pur
# --------------------------------------------------------------------------- #
...
def _add_source_args(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument(
        "--tocodify-file",
        default=None,
        help="Parquet de l'ensemble réellement à coder (classify-regex). Sert à "
