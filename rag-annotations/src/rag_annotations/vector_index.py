...
⚠️  Ce fichier est **dupliqué à l'identique** dans `rag_notices` et
`rag_annotations`. Les deux modules n'ont aucune dépendance commune
aujourd'hui (le module `common/` du chantier 2 n'existe pas encore) et les
faire dépendre de `prune-codes` pour si peu serait sémantiquement faux. Toute
modification doit être reportée dans les deux copies : le consommateur relit
le manifeste écrit par le constructeur, donc un écart de schéma ou de règle de
nommage casse la validation au démarrage.
"""
