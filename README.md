...
- Contenu : accuracy par niveau COICOP (1 à 4) pour **LCS, RAG notices, RAG annotations, TTC, conciliation**, en convention stricte ; accuracy par `shop`, `shop_type_name` et quartile de `budget` ; matrice de confusion ; calibration (accuracy par bucket de confiance, AUROC) ; coût et latence de l'arbitrage LLM. Avec `-p eval-source-column=…`, une ventilation **par provenance du produit**
