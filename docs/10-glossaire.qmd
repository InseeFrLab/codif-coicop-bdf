...
| **LLM-as-judge** | Arbitrage final : un LLM compare les prédictions des quatre classifieurs et tranche ([reconcile-llm](06-reconcile-llm.qmd)). |
| **Conciliation** | Étape qui choisit le code final parmi les candidats des quatre classifieurs. Deux implémentations, **exclusives** : [`reconcile-llm`](06-reconcile-llm.qmd), un juge LLM, et [`reconcile-sirus`](06b-reconcile-sirus.qmd), une vingtaine de règles à seuils lisibles. |
...
| **SIRUS** | *Stable and Interpretable RUle Set* — modèle de conciliation par règles. Entraîné **hors pipeline** (`reconcile-sirus/train.sh`), livré par `reconcile-sirus-model-uri` ([SIRUS](06b-reconcile-sirus.qmd)). |
| **Run smoke** | Passe préalable du DAG complet sur 100 lignes, jouée automatiquement avant chaque run réel : si elle échoue, le vrai run ne démarre pas. `run_id` et espace MLflow distincts. Contournement : `skip-smoke` ([Généralités](00-generalites.qmd)). |
...
| **`contracts.yaml`** | Registre, à la racine du dépôt, des artefacts échangés entre étapes : `inputs` et `outputs` de chacune. Une étape ne construit plus le chemin d'une autre, elle le demande (`codif_common.contracts.artifact()`). En YAML parce que `classify-lcs` est en R et doit le lire aussi ([Généralités](00-generalites.qmd)). |
