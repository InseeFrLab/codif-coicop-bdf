...
| Colonnes utilisateur | `…/{run}/build-datasets/observations.parquet` | `--extra-columns-file` |
...
les **deux** versions, et c'est la version canonique que l'étape [`evaluate`](11-evaluate.qmd) score.

| Colonne | Contenu | Usage |
|---|---|---|
| `code` | code annoté **brut**, tel que saisi (souvent niveau 5) | traçabilité, inspection d'un cas |
| `code_lvl4` | sa forme **canonique** : tronquée niveau 4 puis élaguée | cible du scoring |

::: {.callout-note}
Conserver `code` brut permet à un contrôle méthodologique de comparer les deux colonnes, et de
mesurer combien d'annotations ont effectivement été ramenées à un code plus agrégé.

C'est `code_lvl4` que l'évaluation score, et son absence la fait **échouer** : elle n'est
produite ici que si `--mapping-file` est passé.
:::
...
Le code annoté (`code`) n'est **pas** injecté dans le prompt, même quand le fichier d'entrée
en porte un : le juge décide à l'aveugle, sans quoi la vérité terrain fuiterait dans les
mesures de l'étape [`evaluate`](11-evaluate.qmd).
:::
...
|---|---|
| `…/{run}/reconcile-llm/predictions.parquet` | `id`, les codes normalisés des 4 classifieurs (`lcs_code`, `rag_code`, `ragann_code`, `ttc_code_1..3` + confiances), `code` (vérité terrain **brute**, présente si `label-column`) et `code_lvl4` (vérité **canonique**), `llm_code` (normalisé), `llm_libelle`, `llm_explication`, `llm_confiance`, `llm_model` (`"consensus"` si court-circuit), `llm_error` (raison si `llm_code` est `NA`) |
