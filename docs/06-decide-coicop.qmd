...
- la **vérité terrain `code`** en mode évaluation (annotée au niveau 5), pour que le
  [report](08-report.qmd) compare pruné vs pruné ;
...
|---|---|
| `…/{run}/decide-coicop/predictions.parquet` | `id`, les codes normalisés des 4 classifieurs (`lcs_code`, `rag_code`, `ragann_code`, `ttc_code_1..3` + confiances), `code` (vérité terrain normalisée, éval), `llm_code` (normalisé), `llm_libelle`, `llm_explication`, `llm_confiance`, `llm_model` (`"consensus"` si court-circuit), `llm_error` (raison si `llm_code` est `NA`) |
