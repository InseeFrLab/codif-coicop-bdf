...
- Code : [`decide-coicop/src/decide_coicop.py`](https://github.com/InseeFrLab/codif-coicop-bdf/blob/main/decide-coicop/src/decide_coicop.py)
  (module autonome `decide-coicop/`)
- Commande : sous-commande `decide-coicop` de `decide-coicop/main.py`
...
| Table de mapping [prune](pruning.qmd) *(optionnel)* | `…/{run}/prune/mapping_lvl4.parquet` | `--mapping-file` |
...
step dépend de `prune` et de `run-rag-annotations`).
...
l'**espace de codes pruné** défini par l'étape [`prune`](pruning.qmd) (troncature niveau 4 +
...
(voir [Lancer une étape](09-lancer-une-etape.qmd#decide-coicop)).
...
➡️ Étape suivante : [9. Final output](07-final-output.qmd)
