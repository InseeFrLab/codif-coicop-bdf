...
chemins (l'entraînement manuel et l'étape `sirus-predict`) l'appellent, et un
...
# Colonne de vérité terrain : la forme canonique produite par `decide-coicop`
...
def features_sha256() -> str:
    ...
    `sirus-predict` le compare au sien : un écart signifie que la feature
...
def _is_proposal(series: pd.Series) -> pd.Series:
    ...
    Rejette les NULL/NaN, les abstentions textuelles (`prune.utils.is_answer`,
...
def build_candidate_table(
    ...
    *,
    ...
    ----------
    merged
        Sortie de ``decide_coicop.load_all_observations(..., mapping_path=...)``.
...
def reconcile_population(
    ...
    `load_all_observations` prend la sortie de `codif-lcs` comme table de base et
