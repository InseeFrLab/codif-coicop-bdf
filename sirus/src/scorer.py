...
de ``sirus/train.sh`` sur son test 20 %.
...
def resolve_model_path(model_uri: str | Path) -> Path:
    ...
    Même mécanique que ``codif-ttc/src/predict.py::_resolve_mlflow_path``, pour
    que ``sirus-model-uri`` se comporte exactement comme ``ttc-model-uri``.
    """
    text = str(model_uri).strip()
    if not text:
        raise ValueError(
            "URI de modèle SIRUS vide. Renseigner `sirus-model-uri` dans "
            "argo/params.yaml : c'est l'URI qu'affiche `sirus/train.sh` en fin "
...
def pick_best(table: pd.DataFrame, proba: np.ndarray) -> pd.DataFrame:
    ...
    **Aucun verdict.** Cette étape livre un code et un score, elle ne dit pas
    ...
    permet à `final-output` et au report de joindre le résultat.
    """
