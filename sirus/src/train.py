"""Mesure, calibration et log MLflow de l'entraînement (`sirus/train.sh`).
...
def build_calibration(test: pd.DataFrame, proba: np.ndarray) -> dict:
    ...
    réglage : le pipeline n'applique aucun seuil, `sirus-predict` livre un code
...
def log_to_mlflow(
    ...
    with mlflow.start_run(run_name=run_name or None) as run:
        ...
        # ttc-model-uri (mlflow-artifacts:/<exp>/<run>/artifacts/model).
