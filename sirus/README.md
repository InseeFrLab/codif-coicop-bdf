...
Alternative au juge LLM (`decide-coicop/`) pour choisir le code COICOP final
...
```
 ...
 └───────────────────────────────┬──────────────────────────────────────┘
                                 ...
                                 ▼
  DANS LE PIPELINE — étape sirus-predict, quand `conciliation: sirus`
  4 classifieurs ──→ sirus-predict (Python pur) ──→ sirus-predict/predictions.parquet
...
run dans lequel il tourne. Le détour par MLflow puis par `sirus-model-uri` est ce
...
retenue pour TTC (`ttc-model-uri`).
...
argo submit argo/codif-pipeline.yaml --parameter-file argo/params.yaml \
  ...
  -p sirus-model-uri=mlflow-artifacts:/12/9f3c.../artifacts/model
```

`decide-coicop` est alors *Skipped* ; `final-output` et `report` lisent
`sirus-predict/`. Les deux conciliations sont exclusives.

## Exploiter le score

`sirus-predict` **n'applique aucun seuil**. La sortie porte, par produit :
...
- **Un changement de RAG-ANN invalide le modèle.** Sur le modèle
  ...
  repointe donc l'essentiel du modèle, en silence. `sirus-predict` compare les
